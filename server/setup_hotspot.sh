...
if [ -d "server" ]; then
    ...
    # Only the webserver profile starts by default; run
    # `sudo docker compose --profile soundboard up -d` separately if needed.
    sudo docker compose --profile webserver up -d
else
