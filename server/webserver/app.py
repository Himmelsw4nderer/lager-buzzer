...
class Buzzer:
    ...
    def to_dict(self):
        return {
            ...
            "avg_rtt": sum(self.roundtrip_times) / len(self.roundtrip_times) if self.roundtrip_times else 0
        }
...
class BuzzEvent:
    ...
    def _calculate_rtt(self):
        ...
        try:
            T1 = self.payload.get("time_sync", 0)  # Server unix timestamp when time_sync was sent
            T2 = self.payload.get("time_sync_received", 0)  # Client millis() when time_sync received
            T4 = self.payload.get("send_timestamp", 0)  # Client millis() when buzz was sent
    ...
    def _calculate_corrected_timestamp(self):
        ...
        try:
            ...
            T3 = self.payload.get("button_press", 0)  # Client millis() - actual button press
...
def handle_buzz_in_round(event, buzzer_id):
    ...
    with round_lock:
        ...
        if current_round["winner"] is None:
            ...
            logger.info(f"First buzz! Winner: {current_round['winner']} at {event.corrected_timestamp}")
        else:
            ...
            if event.corrected_timestamp < winner_event.corrected_timestamp:
                current_round["winner"] = buzzer_id or "unknown"
                logger.info(f"New winner: {current_round['winner']} at {event.corrected_timestamp} (was {winner_event.buzzer_id} at {winner_event.corrected_timestamp})")
        ...
        if current_round["winner"]:
            ...
            if buzzer:
                ...
                return jsonify({
                    ...
                    "corrected_timestamp": event.corrected_timestamp if event else None,
                    "roundtrip_time_ms": event.roundtrip_time if event else None,
                    "button_press": event.payload.get("button_press") if event else None,
                    "send_timestamp": event.payload.get("send_timestamp") if event else None
                })
