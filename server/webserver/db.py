...
def create_or_update_buzzer(
    ...
    """Create or update a buzzer in the database."""
    with get_db_cursor() as cursor:
        existing = get_buzzer(client_id)
        ...
        if existing:
            ...
            # Return the updated buzzer
            return get_buzzer(client_id)
        else:
            # Create new buzzer
            cursor.execute(
                """INSERT INTO buzzers (client_id, ip_address, name, enabled, buzz_count, color)
                   VALUES (?, ?, ?, ?, ?, ?)""",
                (
                    client_id,
                    ip_address,
